...
namespace Gley.DailyRewards.Internal
{
    public class CalendarPopup : MonoBehaviour
    {
        ...
        private void Start()
        {
            if(titleText != null)
            {
                titleText.text = Gley.Localization.API.GetText(Localization.WordIDs.DailyReward_TitleText);
            }
        }
        ...
        private void RefreshText()
        {
            ...
            {
                subtitleText.text = Gley.Localization.API.GetText(Localization.WordIDs.DailyReward_Subtitle);
                remainingTime.text = CalendarManager.Instance.GetRemainingTime();
            }
            else
            {
                subtitleText.text = Gley.Localization.API.GetText(Localization.WordIDs.DailyReward_Subtitle);
                remainingTime.text = Gley.Localization.API.GetText(Localization.WordIDs.DailyReward_remainingTime);
