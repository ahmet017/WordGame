...
namespace WordLink
{
	public class UIController : SingletonComponent<UIController>
	{
		...
		[SerializeField] private Text			playButtonText, dailyRewardButtonText, videoRewardText = null;
		...
		public void UpdateUI()
		{
			if (!GameController.Instance.IsLastLevelInGameCompleted())
			{
				...
				playButtonText.text = API.GetText(WordIDs.MainScreen_Playtext) +" "+ (GameController.Instance.LastCompletedLevelNumber + 1);

				
			}

			dailyRewardButtonText.text = API.GetText(WordIDs.DailyReward_ButtonText);
			videoRewardText.text = API.GetText(WordIDs.VideoRewardText_MainMenu);
			//RefeshtopBarLevelText();
			UpdatePackListItems();
		...
		public void OnNewLevelStarted()
		{
			...
			topBarLevelText.text = API.GetText(WordIDs.MainScreen_Playtext)+" " + GameController.Instance.CurrentActiveLevel.levelData.GameLevelNumber;
		...
		public void RefeshtopBarLevelText()
        {
			topBarLevelText.text = API.GetText(WordIDs.MainScreen_Playtext ) + " " + GameController.Instance.CurrentActiveLevel.levelData.GameLevelNumber;
		}
